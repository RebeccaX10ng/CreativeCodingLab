...
class Notes {
    ...
    update() {
        ...
        let shouldPlayX = (this.noteX < 0 || this.noteX > width - this.noteImage.width * this.noteScale) && !audioFiles[this.randomIndex].isPlaying();
        let shouldPlayY = (this.noteY < 0 || this.noteY > height - this.noteImage.height * this.noteScale) && !audioFiles[this.randomIndex].isPlaying();

        if (shouldPlayX || shouldPlayY) {
